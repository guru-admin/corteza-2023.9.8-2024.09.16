...
ARG VERSION=2023.9.8
ARG SASS_VERSION=1.69.5
ARG SERVER_VERSION=${VERSION}
ARG WEBAPP_VERSION=${VERSION}
ARG CORTEZA_SERVER_PATH=/home/ubuntu/original/corteza/corteza-server--linux-amd64.tar.gz
...
COPY corteza-server--linux-amd64.tar.gz /tmp/server
